...
# The split manifest is auto-generated by generate_dataset.py and written to
# data/processed/split_manifest.json. This replaces the previously hardcoded
# SPLIT_MANIFEST dict. Changing one attack type's campaign count can no longer
# silently desync this file — it is always freshly computed from actual data.

_SPLIT_MANIFEST_PATH = "data/processed/split_manifest.json"

def _load_split_manifest() -> dict:
    """Load the split manifest from the auto-generated JSON file."""
    if os.path.exists(_SPLIT_MANIFEST_PATH):
        with open(_SPLIT_MANIFEST_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    else:
        raise FileNotFoundError(
            f"Split manifest not found at {_SPLIT_MANIFEST_PATH}. "
            "Run src/ingest/generate_dataset.py first to generate it."
        )
