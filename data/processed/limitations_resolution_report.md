...
## Part 1 Post-Check Verification

*Added 2026-07-25. Read-only verification of the Part 1 dataset refinement and pipeline retrain.*

---

### Check 1 — Explicit Normal False Positive Count

From `data/processed/fused_scores.parquet` on the test split ($n=3,289$ total test sessions):

| Metric | Last Verified Baseline | Part 1 Retrain | Change |
|--------|------------------------|----------------|--------|
| Normal Test Sessions | 3,162 | 3,167 | +5 |
| Normal False Positives (FP) | 47 | **13** | **-34 (-72.3%)** |
| Normal False Positive Rate (FPR) | 1.49% | **0.41%** | **-1.08%** |
| Precision | 0.708 | **0.902** | **+0.194** |
| Recall | 0.908 | **0.984** | **+0.076** |
| F1 Score | 0.796 | **0.941** | **+0.145** |

**Verdict on Check 1:** **CONFIRMED.** The normal FP count dropped from 47 to 13 (a 72.3% reduction in false positives), dropping the FP rate from 1.49% to 0.41%. This confirms that the Part 1 dataset refinement achieved a simultaneous improvement in precision, recall, and false positive suppression.

---

### Check 2 — Impossible Travel Test Coverage Analysis

#### 1. Test Session Inventory for `impossible_travel`
The test split contains 3 `impossible_travel` sessions:

| Session ID | Campaign ID | Prefix | `fp_mismatch` | `geo_velocity_violation` | Fused Score | Tier | Detected |
|------------|-------------|--------|---------------|--------------------------|-------------|------|----------|
| `SESS_ITSC_f87b85bf` | `ATK_ITSC_20260619_007` | `ATK_ITSC_` | 0 | 1 | 93 | 1 | Yes |
| `SESS_ITSC_681d3777` | `ATK_ITSC_20260618_003` | `ATK_ITSC_` | 0 | 1 | 93 | 1 | Yes |
| `SESS_IMP_0b267adc` | `ATK_IT_20260618_001` | `ATK_IT_` | 1 | 1 | 96 | 1 | Yes |

- **Sub-variant Representation:** **BOTH sub-variants are present in test.** 2 out of 3 sessions belong to the stolen-credential sub-variant (`ATK_ITSC_`, `fp_mismatch=0`), and 1 belongs to the original sub-variant (`ATK_IT_`, `fp_mismatch=1`).
- **Detection Result:** Both stolen-credential ITSC test sessions were detected at Tier 1 with fused risk score = 93.

#### 2. `auto_split_manifest()` Pooling Logic
In `src/ingest/generate_dataset.py`, `auto_split_manifest()` groups campaigns strictly by `attack_type`:
```python
...
```
Because both `ATK_IT_` and `ATK_ITSC_` campaigns share `attack_type = 'impossible_travel'`, `auto_split_manifest()` pools all 20 campaigns together (10 `ATK_IT_` and 10 `ATK_ITSC_`) and selects the 3 latest-dated campaigns overall. In this generation run, chronological sorting selected 2 `ATK_ITSC_` campaigns and 1 `ATK_IT_` campaign into test.

#### 3. Verification & Fix Recommendation
- **Current Coverage Status:** **INTACT.** The reported recall of 1.000 (3/3) for `impossible_travel` directly evaluates and confirms detection of the stolen-credential scenario (`fp_mismatch=0`, `geo_velocity_violation=1`).
- **Recommended Hardening (for future runs):** To prevent future random seed or campaign date shifts from pooling only one prefix into test, `auto_split_manifest()` should be updated to allocate test campaigns per prefix (`ATK_IT_` vs `ATK_ITSC_`) or reserve 2 test campaigns per sub-variant (total n=4 test campaigns).

---

### Final Summary
- **FP Count Check:** Confirms dramatic improvement (47 → 13 FPs, 0.41% FPR).
- **Impossible Travel Coverage:** Intact and verified (2/3 test sessions are stolen-credential ITSC).
- **Part 1 Status:** **FULLY VERIFIED.**


## Split Manifest Hardening

*Added 2026-07-25. Structural fix to `auto_split_manifest()` in `src/ingest/generate_dataset.py`.*

---

### Implementation & Verification

`auto_split_manifest()` has been updated to detect campaign ID prefixes generically (e.g., `ATK_IT_` vs `ATK_ITSC_`) before allocating test campaigns:

- **Single-Prefix Attack Types (6 classes):** Standard behavior remains unchanged — the 3 latest-dated campaigns overall go to test (7 train / 3 test).
- **Multi-Prefix Attack Types (`impossible_travel`):** The function now groups campaigns by prefix first and reserves 2 latest-dated campaigns **per prefix** for the test split (4 test campaigns total).

### Verification Summary
1. **Manifest Re-generation:** `data/processed/split_manifest.json` was updated against current telemetry data.
2. **Sub-variant Guarantee:** `impossible_travel` test split now structurally guarantees 4 test campaigns: 2 `ATK_IT_` (original) + 2 `ATK_ITSC_` (stolen-credential).
3. **No Unintended Side Effects:** All other 6 attack categories retained exactly 3 test campaigns (7 train / 3 test).
